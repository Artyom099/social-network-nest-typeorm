...
export class CommentsQueryRepository {
  ...
  async getCommentsCurrentPost(
    ...
    offset $3
    `,
      [postId, query.pageSize, query.offset()],
    );

    const items = await Promise.all(
      sortedComments.map(async (c): Promise<CommentViewModel> => {
        const [myLikeInfo] = await this.dataSource.query(
          `
      select *
      from "comment_likes"
  ...
  async getCommentsCurrentBlogger(
    ...
    query: DefaultPaginationInput,
  ): Promise<Pagination<BLoggerCommentViewModel[]>> {
    const [totalCount] = await this.dataSource.query(
      `
    select count (*)
    from "comments" c
    ...
    offset $3
    `,
      [currentUserId, query.pageSize, query.offset()],
    );
